...
function resolveHttpOrigin(url: string | undefined): string | null {
  ...
  try {
    ...
    // En producción el navegador debe usar /api/wa-proxy (proxy Worker), no la IP del VPS.
    if (/^\d{1,3}(\.\d{1,3}){3}$/.test(hostname)) {
      console.warn(
        '[sie-csp] VITE_OPENWA_API_URL apunta a una IP. Use /api/wa-proxy en Cloudflare para evitar ERR_CERT_AUTHORITY_INVALID.'
      );
      return null;
    }
    return origin;
 ...
 * Rutas relativas (/api/wa-proxy) usan 'self' y no requieren entrada extra.
 */
